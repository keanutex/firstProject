...
# Connect files and add self.color
    def connectFilesAddColour(self,evecCol1,evecCol2,evecCol3,pheCol):
        ...
        self.categories = catagory

#-------------------------------------------------------------------------------------------------------
    def plotGraph(self):
#Plot as graph

        colorHex = ['#C0C0C0', '#FF0000', '#FFFF00', '#00FF00', '#008000', '#00FFFF', '#008080', '#0000FF', '#FF00FF',
                '#800080', '#008000', '#000000']
        fig = plt.figure()
        ...
        for i in range(len(self.categories)):
            ...
            ax.scatter(x, y, c = colorHex[i], s = 10, label = self.categories[i], alpha=1)

        ax.scatter3D(self.col1, self.col2, self.col3, s=10, c=self.color);

        ax.legend()
        plt.show()


    def getMaxColumnEvec(self):
        return len(self.coloumnEvec)

    def getMaxColumnPheno(self):
